...
class HealthBarView:
    ...
    def update_health(self, health: int):
        ...
        bg_healthbar_size = (self.max_bar_length + 4, health_bar_height + 4)  # Tamanho da barra de fundo (branca)
        health_bar_size = (self.max_bar_length, health_bar_height)  # Tamanho da barra de fundo (vermelha)

        # Posição da barra de fundo (branca)
        position_x = self.position.x + (bg_healthbar_size[0] / 2) - (self.max_bar_length / 2)
        position_y = self.position.y + (bg_healthbar_size[1] / 2) - (health_bar_height / 2)

        # Desenhar o fundo da barra de saúde (branca)
        self.rectangle_renderer.draw((255, 255, 255), Vector2(position_x, position_y), bg_healthbar_size)

        # Desenhar a barra de fundo (vermelha) que é um pouco menor
        self.rectangle_renderer.draw((255, 0, 0), Vector2(position_x + 2, position_y + 2), (self.max_bar_length, health_bar_height))

        # Posição da barra de saúde (amarela)
        position_health_bar_x = position_x + 2  # Ajuste para manter alinhamento com a barra vermelha
        position_health_bar_y = position_y + 2  # Ajuste para manter alinhamento com a barra vermelha
        dimensions_health = (bar_length, health_bar_height)  # Tamanho da barra de saúde (amarela)

        # Ajustar a posição da barra de saúde (amarela) para que diminua da direita para a esquerda, se necessário
        if self.reverse:
            position_health_bar_x += (self.max_bar_length - bar_length)  # Para o jogador 2, ajusta a posição

        # Desenhar a barra de saúde (amarela)
        self.rectangle_renderer.draw(color, Vector2(position_health_bar_x, position_health_bar_y), dimensions_health)
